 ...
 * @returns {number}
 */
function artifact_format_version() {
    const ret = wasm.artifact_format_version();
    return ret >>> 0;
}
exports.artifact_format_version = artifact_format_version;
 ...
 * @returns {string}
 */
function compile(modules_json, root_target) {
    let deferred4_0;
    let deferred4_1;
    try {
 ...
 * @returns {string}
 */
function execute(artifact_json, request_json) {
    let deferred4_0;
    let deferred4_1;
    try {
...
const wasmPath = `${__dirname}/axiom_rules_engine_wasm_bg.wasm`;
